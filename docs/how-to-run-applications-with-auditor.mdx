...
### Decide whether to purge residual transaction state

Over time, completed transactions leave behind residual transaction state (Coordinator state records and request proofs) that consumes storage. You can configure ScalarDL to purge this state automatically or on demand. Because enabling or changing purge requires restarting Ledger and Auditor, decide on this before you start them. For details, see [Purge Residual Transaction State](purge-residual-transaction-state.mdx).
