...
```markdown
Set 'scalar.dl.auditor.transaction_state_purge.enabled' to true in the Auditor configuration (for example, auditor.properties) and restart Auditor to allow purge.
```
...
```markdown
Upgrade the request proof table schema to the version that supports transaction state purge and restart Auditor. Alternatively, disable transaction state purge by setting 'scalar.dl.auditor.transaction_state_purge.enabled' to false in the Auditor configuration (for example, auditor.properties).
```
