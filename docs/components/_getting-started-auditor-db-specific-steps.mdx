...
1. Enable the container image for the Enterprise edition in the <code>{props.shortName}/docker-compose-ledger.yml</code> file as follows:
...
3. To validate the license by using a certificate, update the <code>{props.shortName}/docker-compose-ledger.yml</code> and <code>{props.shortName}/docker-compose-auditor.yml</code> files as follows. If you're using a trial license, skip this step.
