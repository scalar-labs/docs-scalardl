...
For how those functionalities are achieved by using generic contracts and functions, see the examples in [Validate object authenticity](#validate-object-authenticity) and [Validate collection authenticity](#validate-collection-authenticity) below.
...
The `sha256sum` command is for Linux environments only. `shasum` and `certutil` are available for Mac and Windows environments, respectively. See usage of those commands for details.
