...
There are two primary use cases for namespaces:

- **Cross-namespace use case:** A single application accesses assets across multiple namespaces.
- **Multi-tenant use case:** Multiple independent applications each have exclusive access to assets within their own namespace.

## Cross-namespace use case

The cross-namespace use case is suitable for scenarios such as the following:
...
The multi-tenant use case is suitable for scenarios such as the following:
...
ScalarDL supports these scenarios through isolated namespaces, where each namespace is independently managed and inaccessible from other namespaces. To set up an isolated namespace, first create a namespace, and then register the certificates or secrets of tenant clients that will use each namespace. Then, register contracts and functions for their application through the registered clients. Only clients registered in a namespace can register and execute contracts and functions or validate assets within that namespace.

Unlike contracts registered in the `default` namespace, contracts registered in isolated namespaces have only a local access scope and cannot access assets in other namespaces.

For details on isolated namespaces, see [Use Isolated Namespaces](use-isolated-namespaces.mdx).

## See also

- [A Guide on How to Write a Good Contract](how-to-write-contract.mdx)
- [Use Isolated Namespaces](use-isolated-namespaces.mdx)
