...
This document explains how to set up and use restricted namespaces in a multi-tenant deployment of ScalarDL.

## What are restricted namespaces?

Restricted namespaces allow you to host multiple tenants on a single ScalarDL cluster while ensuring strict data isolation between them. Each tenant operates within its own namespace and can only access assets within that namespace.

### Security model

The restricted namespace feature relies on port-based access control:
