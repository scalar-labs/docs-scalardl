...
To execute some tools for deploying and managing ScalarDB Server on AKS, you must prepare a bastion server in the same Azure Virtual Network (VNet) of the AKS cluster that you created in **Step 1**.  For details, see [Create a Bastion Server](CreateBastionServer.mdx).
