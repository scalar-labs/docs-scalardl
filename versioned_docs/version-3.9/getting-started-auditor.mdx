...
* [A Guide on How to Use Asset Proofs in ScalarDL](how-to-use-proof.mdx)
...
Note that you also need to enable `scalar.dl.ledger.proof.enabled` and set a proper private key to `scalar.dl.ledger.proof.private_key_path` since Auditor uses [AssetProof](how-to-use-proof.mdx) to work.
