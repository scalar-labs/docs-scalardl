...
このページには、ScalarDL 3.5 のリリースノートのリストが含まれています。
...
- 新しいコントラクト I/F に一致する新しい Ledger インターフェースを追加しました。
- ジャクソンベースのコントラクトと台帳を追加しました。
- 非推奨クラスには `DeprecatedLedgerReturnable` を使用しました。
- アプリケーションが `ClientService` API を呼び出すための JSON ライブラリを公開しました。
- 新しい Contract/Ledger インターフェースでの下位互換性をチェックするための E2E テストを追加しました。
- `master` ブランチに変更されました。
