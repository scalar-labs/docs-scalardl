...
ScalarDL を操作するには、2つのオプションがあります。[ScalarDL Ledger をはじめよう](getting-started.mdx)に示されている[コマンド](scalardl-command-reference.mdx)を使用するか、[Java Client SDK](https://javadoc.io/doc/com.scalar-labs/scalardl-java-client-sdk/latest/index.html) を使用するかです。
...
詳細については、<JavadocLink packageName="scalardl-java-client-sdk" path="com/scalar/dl/ledger/service" className="StatusCode" /> を参照してください。
...
Ledger のみを使用する場合、有効な状態とは、データが格納されているハッシュチェーン構造が有効であることを意味します。そのため、悪意のあるユーザーがデータの一部を変更した場合、構造検証によって変更を検知できます。ただし、悪意のあるユーザーが Ledger プログラム自体を変更した場合、たとえば構造検証コードも変更できるため、変更を検知できない可能性があります。

Ledger と Auditor を使用する場合、有効な状態とは、Ledger と Auditor のデータに矛盾がないことを意味します。そのため、悪意のあるユーザーが Ledger プログラムを変更した場合でも、Auditor が正しく、変更がデータまたは出力に影響する限り、変更が検知されます。Auditor は、実行ごとに、Ledger と Auditor が参照した状態と作成した状態の矛盾をチェックします。したがって、これらの状態は実行時に正しいことが保証されます。

この違いにより、Ledger のみを使用する場合と Ledger と Auditor の両方を使用する場合で検証の内容が異なります。Ledger のみを使用する場合、検証はアセットをトラバースして、アセットが再計算可能で、有効なハッシュチェーン構造を持っているかどうかを確認します。Ledger と Auditor を使用する場合、検証は Ledger と Auditor の状態間の矛盾を中央集権的な調整なしでチェックします。
...
`validateLedger` メソッドを使用して検証を行うことができます。ただし、Ledger のみを使用する場合と Ledger と Auditor の両方を使用する場合で、エラーハンドリングの動作が異なります。

#### Ledger のみを使用する場合

Ledger のみを使用する場合、`validateLedger` は検証失敗時に例外をスロー**しません**。代わりに、返された `LedgerValidationResult` のステータスコードを確認して、データが有効かどうかを判定する必要があります。
...
検証に関連する主なステータスコードは次のとおりです。

- `INVALID_HASH`: アセットレコードのハッシュ値が期待値と異なります。
- `INVALID_PREV_HASH`: 前のハッシュ値が期待値と異なります。
- `INVALID_CONTRACT`: 以前に実行されたコントラクトが無効なアセットレコードを作成しました。
- `INVALID_OUTPUT`: アセットレコードのデータ値が期待値と異なります。

#### Ledger と Auditor を使用する場合

Ledger と Auditor を使用する場合、`validateLedger` は Ledger と Auditor の間で不整合を検知すると `ClientException` をスローします。例外をキャッチし、そのステータスコードを確認する必要があります。
...
Auditor ベースの検証に関連する主なステータスコードは次のとおりです。

- `INCONSISTENT_STATES`: Ledger と Auditor の間で状態が不整合であり、改ざんの可能性があります。

Ledger のみを使用する場合、アセットプルーフと呼ばれる情報を使用することで検知機能を強化できます。Ledger と Auditor を使用する場合、アセットプルーフを明示的に使用する必要はありませんが、Auditor は内部的にこれを使用してビザンチン故障検知機能を実現します。Auditor の詳細については、[ScalarDL 設計ドキュメント](design.mdx)を参照してください。
...
アセットプルーフは、Client SDK の `executeContract` メソッドの結果から <JavadocLink packageName="scalardl-java-client-sdk" path="com/scalar/dl/ledger/proof" className="AssetProof" /> として取得できます。署名を検証することで、改ざんされておらず Ledger からのアセットプルーフであることが検証できます。

Ledger が実行されるドメインの外部にアセットプルーフを保存することをお勧めします。これは、あるドメインでの悪意のあるアクティビティを別のドメインで検知できるようにするためです。管理を容易にするために、クラウドストレージにアセットプルーフを保存することも検討する価値があります。
