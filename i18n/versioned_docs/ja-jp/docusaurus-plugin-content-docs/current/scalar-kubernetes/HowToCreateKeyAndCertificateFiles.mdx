...
ScalarDLにはいくつかの種類の認証方式があります。 認証方法として `digital-signature` を使用する場合は、秘密鍵と証明書ファイルを準備する必要があります。 認証方法の詳細については、[ScalarDL 認証ガイド](https://scalardl.scalar-labs.com/ja-jp/docs/latest/authentication/)を参照してください。
