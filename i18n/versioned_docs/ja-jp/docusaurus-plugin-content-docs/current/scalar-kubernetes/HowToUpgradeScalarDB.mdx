...
ScalarDB Cluster を新しいバージョンにアップグレードする前に、ScalarDB Cluster とクライアント SDK 間の互換性を確保するために次のルールを確認してください。
...
- ScalarDB Cluster とクライアント SDK の **メジャー** バージョンが異なる場合、それらは互換性がないため、**サポートされていません**。
