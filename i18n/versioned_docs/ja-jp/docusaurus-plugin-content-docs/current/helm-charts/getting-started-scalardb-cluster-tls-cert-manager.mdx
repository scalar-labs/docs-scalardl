...
1. ライセンスキーと証明書を環境変数として設定します。ライセンスキーがない場合は、[お問い合わせ](https://www.scalar-labs.com/contact)ください。`<CERT_PEM_FOR_YOUR_LICENSE_KEY>` の値の詳細については、[ライセンスキーの設定方法](../scalar-licensing/index.mdx)を参照してください。
