...
* ScalarDLのライセンスキー（試用ライセンスまたは商用ライセンス）を取得する必要があります。 ライセンス キーをお持ちでない場合は、[お問い合わせ](https://www.scalar-labs.com/contact) してください。
...
1. PostgreSQL for Auditor のユーザー名とパスワードを含むシークレット リソースを作成します。
...
1. Ledger 用の ScalarDL スキーマ ローダーをデプロイします。
...
1. Auditor 用の ScalarDL スキーマ ローダーをデプロイします。
...
1. 資格情報とライセンスキーを含むシークレット リソース `ledger-credentials-secret` を作成します。
...
1. 資格情報とライセンスキーを含むシークレット リソース `auditor-credentials-secret` を作成します。
...
1. Envoy の秘密キーファイルと証明書ファイルを含むシークレット リソースを作成します。
...
1. ScalarDL Ledger の秘密キー、証明書、CA 証明書ファイルを含むシークレット リソースを作成します。
...
1. ScalarDL Auditor の秘密キー、証明書、CA 証明書ファイルを含むシークレット リソースを作成します。
...
1. 秘密リソース `auditor-keys` を作成して、`デジタル署名` 認証方法を無効にします。 このドキュメントでは、 `デジタル署名` の代わりに `hmac` 認証方法を使用します。
...
* [ScalarDL 入門](https://github.com/scalar-labs/scalardl/blob/master/docs/getting-started.md)
* [ScalarDL Auditor 入門](https://github.com/scalar-labs/scalardl/blob/master/docs/getting-started-auditor.md)
