...
- **クライアントをブートストラップする**
  - [`bootstrap`](#bootstrap): ID 情報とシステムコントラクトを登録してクライアントをブートストラップします。
- **ID 情報を登録する**
...
- **名前空間を管理する**
  - [`create-namespace`](#create-namespace): 名前空間を作成します。
  - [`list-namespaces`](#list-namespaces): 名前空間を一覧表示します。
  - [`drop-namespace`](#drop-namespace): 名前空間を削除します。
- **トランザクション状態を管理します**
  - [`purge-state`](#purge-state): ScalarDL が保持する残留トランザクション状態をパージします。
- **台帳を検証します**
...
1. プロパティファイルで設定された認証方式に基づいて、証明書またはシークレットを登録します。
2. Auditor が有効な場合、`ValidateLedger` コントラクトを登録します。
...
プロパティファイルの認証情報を使用してシークレットを登録する。
...
名前空間を作成します。名前空間名は英字で始まり、英数字とアンダースコアのみを含むことができます (パターン: `[a-zA-Z][a-zA-Z0-9_]*`)。デフォルトの名前空間である `default` は予約されており、作成または削除することはできません。
...
|:---------------------------|:--------------------------|
| `--config`, `--properties` | プロパティ形式の設定ファイル。 |
| `--namespace`              | 作成する名前空間の名前。      |

[共通ユーティリティオプション](#共通ユーティリティオプション)も利用可能です。
...
|:---------------------------|:-----------------------------------------|
| `--config`, `--properties` | プロパティ形式の設定ファイル。                |
| `--pattern`                | 名前空間をフィルタリングするパターン（部分一致）。 |
...
名前空間を削除します。このコマンドは、確認のために名前空間名を入力する必要があります。
...
ScalarDL が完了したトランザクションについて保持している残留トランザクション状態 (Coordinator state レコードと request proof) をパージします。このコマンドは、Auditor が有効で、かつ Ledger と Auditor の両方でトランザクション状態のパージが有効 (`scalar.dl.ledger.transaction_state_purge.enabled` と `scalar.dl.auditor.transaction_state_purge.enabled` が `true`) になっている場合にのみ利用できます。詳細については、[残留トランザクション状態のパージ](purge-residual-transaction-state.mdx)を参照してください。

デフォルトでは、このコマンドはパージ前に確認を求めます。確認プロンプトをスキップするには (たとえば非対話環境で実行する場合)、`--force` オプションを使用します。
...
|:---------------------------|:-------------------------------------------------|
| `--config`, `--properties` | プロパティ形式の設定ファイル。                        |
| `-f`, `--force`            | 確認を求めずに残留トランザクション状態をパージします。       |

[共通ユーティリティオプション](#共通ユーティリティオプション)も利用できます。

### 例

残留トランザクション状態をパージします。実行前に確認を求められます。

```console
scalardl purge-state --properties client.properties
```

確認プロンプトなしで残留トランザクション状態をパージします。

```console
scalardl purge-state --properties client.properties --force
```

このコマンドは、対象となったトランザクション、パージされたトランザクション、およびスキップされたトランザクションの数のサマリーを返します。
...
|:---------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------|
| `--config`, `--properties` | プロパティ形式の設定ファイル。                                                                                                                        |
| `--namespace`              | アセットの名前空間。指定されていない場合は、プロパティファイルの`scalar.dl.client.context.namespace`値が使用されます。それも設定されていない場合は、`default`名前空間が使用されます。 |
...
汎用コントラクトは ScalarDL 3.10 で導入されましたが、ScalarDL 3.12 でリリースされた HashStore は、汎用コントラクトをラップしたより高次の抽象化を提供します。ほとんどのユースケースでは、汎用コントラクトを直接使用するよりも、HashStore を使用する方がシンプルで効率的です。詳細については、ScalarDL の最新版の [ScalarDL HashStore をはじめよう](./getting-started-hashstore.mdx)をご参照ください。
