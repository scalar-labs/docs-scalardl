...
- **フィールド:** `scalar.dl.ledger.function.non_privileged_port_registration.enabled`
- **説明:** 非特権ポートを通じたファンクション登録を有効にするオプション。`true` に設定すると、テナントが非特権ポートを通じてファンクションを登録できるようになります。
- **デフォルト値:** `false`

### `function.non_privileged_port_registration.overwrite.enabled`

- **フィールド:** `scalar.dl.ledger.function.non_privileged_port_registration.overwrite.enabled`
- **説明:** 非特権ポートを通じたファンクション登録時に既存のファンクションの上書きを許可するオプション。`true` に設定すると、テナントが非特権ポートを通じて既存のファンクションを上書きできるようになります。
- **デフォルト値:** `false`
...
- **フィールド:** `scalar.dl.ledger.transaction_state_purge.enabled`
- **説明:** Ledger 側で Coordinator state レコードをパージするためのマスタースイッチ。これが `false` に設定されている間、Ledger はトランザクション状態のパージリクエストを拒否し、Coordinator state レコードが意図せず削除されないようにします。これは `scalar.dl.auditor.transaction_state_purge.enabled` とあわせて有効にする必要があります。トランザクション状態のパージは Auditor が有効な場合にのみサポートされます。詳細については、[残留トランザクション状態のパージ](purge-residual-transaction-state.mdx)を参照してください。
- **デフォルト値:** `false` (オプション)
...
- **フィールド:** `scalar.dl.auditor.transaction_state_purge.enabled`
- **説明:** 残留トランザクション状態 (Coordinator state レコードと request proof) をパージするためのマスタースイッチ。これが `false` に設定されている間は、状態はパージされず、手動パージのリクエストは拒否されます。詳細については、[残留トランザクション状態のパージ](purge-residual-transaction-state.mdx)を参照してください。
- **デフォルト値:** `false` (オプション)

### `transaction_state_purge.on_completion.enabled`

- **フィールド:** `scalar.dl.auditor.transaction_state_purge.on_completion.enabled`
- **説明:** 各トランザクションの残留状態をトランザクション完了直後にパージする、完了時パージを有効にするフラグ。これは `scalar.dl.auditor.transaction_state_purge.enabled` が `true` に設定されている場合にのみ有効です。
- **デフォルト値:** `false` (オプション)

### `transaction_state_purge.scan.interval_secs`

- **フィールド:** `scalar.dl.auditor.transaction_state_purge.scan.interval_secs`
- **説明:** 定期パージがバックグラウンドで残留トランザクション状態をスキャンしてパージする間隔 (秒単位)。これが `0` に設定されている場合、定期パージは実行されませんが、手動パージは引き続き利用できます。これは `scalar.dl.auditor.transaction_state_purge.enabled` が `true` に設定されている場合にのみ有効です。
- **デフォルト値:** `0` (オプション)

### `transaction_state_purge.scan.limit`

- **フィールド:** `scalar.dl.auditor.transaction_state_purge.scan.limit`
- **説明:** 定期パージまたは手動パージの際に、1回のスキャンで処理するレコードの最大数。
- **デフォルト値:** `1000` (オプション)
