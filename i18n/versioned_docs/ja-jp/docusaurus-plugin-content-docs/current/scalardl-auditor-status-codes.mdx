...
```markdown
Set 'scalar.dl.auditor.transaction_state_purge.enabled' to true in the Auditor configuration (for example, auditor.properties) and restart the Auditor to allow purge.
```
...
```markdown
Transaction state purge is enabled, but the request proof table schema is too old to support it. With this schema, request proofs are stored in a legacy format that can never be purged, so they would accumulate indefinitely.
```

**対処法**

```markdown
Upgrade the request proof table schema to the version that supports transaction state purge and restart the Auditor. Alternatively, disable transaction state purge by setting 'scalar.dl.auditor.transaction_state_purge.enabled' to false in the Auditor configuration (for example, auditor.properties).
```
...
```markdown
Deleting the request proof failed. Details: %s
```
...
```markdown
Scanning request proofs failed. Details: %s
```
...
```markdown
Getting the request proof table metadata failed. Details: %s
```
...
```markdown
Calling Ledger failed. Status code: %s; Details: %s
```

**対処法**

```markdown
Ledger may be temporarily unavailable. Check the error details in the logs, the availability of Ledger, and the connection and authentication settings between Ledger and Auditor.
```
...
```markdown
Another transaction state purge is already in progress.
```

**対処法**

```markdown
Retry the operation after the current purge completes.
```
