...
※コンテナイメージ (`scalardb-server` および `scalardb-envoy`) を取得するには、[AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-rzbuhxgvqf4d2)。AWS を使用している場合、詳細については [AWS Marketplace を通じて Scalar 製品をインストールする方法](../scalar-kubernetes/AwsMarketplaceGuide.mdx)を参照してください。
...
1. AWS から ScalarDB コンテナーイメージをプルするためのシークレットリソースを作成します。
   * AWS Marketplace
     ```console
