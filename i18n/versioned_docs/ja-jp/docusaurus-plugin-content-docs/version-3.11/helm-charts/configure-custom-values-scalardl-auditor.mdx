...
`auditor.auditorProperties` を設定する必要があります。`auditor.properties` をこのパラメータに設定してください。詳細については、[ScalarDL Auditor の設定](../configurations.mdx#auditor-の設定)を参照してください。
...
秘密鍵ファイルを `scalar.dl.auditor.private_key_path` に設定する必要があります。

秘密鍵ファイルを ScalarDL Auditor ポッドにマウントする必要があります。

秘密鍵ファイルをマウントする方法の詳細については、[ScalarDL Helm Charts のポッドに秘密鍵ファイルをマウントする](mount-files-or-volumes-on-scalar-pods.mdx#scalardl-helm-charts-のポッドに秘密鍵ファイルをマウントする)を参照してください。
