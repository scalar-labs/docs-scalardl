...
- **説明:** 非特権ポートを通じたファンクション登録を有効にするオプション。`true` に設定すると、クライアントが非特権ポートを通じてファンクションを登録できるようになります。
- **デフォルト値:** `false`
...
- **説明:** 非特権ポートを通じたファンクション登録時に既存のファンクションの上書きを許可するオプション。`true` に設定すると、クライアントが非特権ポートを通じて既存のファンクションを上書きできるようになります。
- **デフォルト値:** `false`
