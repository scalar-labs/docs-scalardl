...
次に、ScalarDL は Asset Proof を使用して Ledger と Auditor 間の整合性をチェックするため、`scalar.dl.ledger.proof.enabled` を `true` に設定して Ledger 設定で [Asset Proof](how-to-write-applications.mdx#asset-proof-とは何か) を有効にする必要があります。また、[認証方法を決定する](#認証方法を決定する)セクションで選択した認証方法に応じて、Asset Proof に署名するために Ledger と Auditor の設定で適切な秘密鍵または秘密キーを設定する必要があります。
...
また、秘密情報の準備も必要です。電子署名方式の場合は証明書と秘密鍵、HMAC 方式の場合は秘密キーを用意してください。ScalarDL の認証について詳しくは [ScalarDL 認証ガイド](./authentication.mdx)を参照してください。
...
このガイドでは、Ledger と Auditor をローカルで起動するために `scalardl-samples` のコンテナベース環境を使用します。リポジトリのクローンが完了していない場合は、[前提条件](getting-started.mdx#前提条件)と[ScalarDL サンプルリポジトリをクローンする](getting-started.mdx#scalardl-サンプルリポジトリをクローンする)を参照してください。
...
## クライアントから証明書または秘密キーを登録する

[認証方式の決定](#認証方式の決定)で選択した認証方法を選択し、コマンドに従って認証を設定してください。
...
HMAC 認証方式の場合、`register-secret` コマンドを使用して秘密キーを登録できます。このコマンドの詳細については、[ScalarDL クライアントコマンドリファレンス](scalardl-command-reference.mdx#register-secret)を参照してください。
...
汎用コントラクトを使用している場合は、[汎用コントラクトおよびファンクションの使用](use-generic-contracts.mdx#汎用コントラクトとファンクションを登録する)を参照してコントラクトとファンクションを登録してください。
