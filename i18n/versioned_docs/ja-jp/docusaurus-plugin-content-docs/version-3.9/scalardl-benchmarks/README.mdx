...
[ScalarDL と Client SDK の互換性](../compatibility.mdx) に基づいて、ScalarDL Java Client SDK のバージョンが対象の ScalarDL Ledger および Auditor でサポートされているバージョンであるかどうかを `build.gradle` ファイルで確認します。サポートされていない場合は、`build.gradle` ファイル内の次の部分の `<VERSION>` を `X.Y.Z` のようなバージョン (たとえば `3.9.6`) に変更します。
