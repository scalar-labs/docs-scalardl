...
テーブル指向汎用コントラクトは ScalarDL 3.11 で導入されましたが、ScalarDL 3.12 でリリースされた TableStore は、テーブル指向汎用コントラクトをラップしたより高次の抽象化を提供します。ほとんどのユースケースでは、テーブル指向汎用コントラクトを直接使用するよりも、TableStore を使用する方がシンプルで効率的です。詳細については、ScalarDL の最新版の [ScalarDL TableStore をはじめよう](./getting-started-tablestore.mdx)をご参照ください。
