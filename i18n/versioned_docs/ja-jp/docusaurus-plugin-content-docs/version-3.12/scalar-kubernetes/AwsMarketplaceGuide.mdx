...
1. Scalar 製品を選択すると、AWS Marketplace へのリンクが表示されます。
   ...
   <Tabs groupId="products" queryString>
     ...
     <TabItem value="scalardb-analytics-server" label="ScalarDB Analytics server">
       | PAYG                                                                                       |
       |:-------------------------------------------------------------------------------------------|
...
1. インストールする Scalar 製品の Helm Chart のカスタム値ファイルを更新します。
   ...
   <Tabs groupId="products" queryString>
     ...
     <TabItem value="scalardb-analytics-server" label="ScalarDB Analytics server">
       <h3>ScalarDB Analytics server</h3>

       `scalardb-analytics-server-custom-values.yaml` ファイル内:
     ...
     設定の詳細については、[ScalarDB Analytics Server のカスタム値ファイルを構成する](../helm-charts/configure-custom-values-scalardb-analytics-server.mdx)を参照してください。
