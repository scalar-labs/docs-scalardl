...
以下のコンテナイメージを取得するには、[AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-rzbuhxgvqf4d2) で ScalarDL Ledger および ScalarDL Auditor をサブスクライブする必要があります。
   * AWS Marketplace
      * scalar-ledger
      * scalar-ledger-envoy
...
1. AWS から ScalarDL コンテナーイメージをプルするためのシークレットリソースを作成します。
   * AWS Marketplace
     ```console
