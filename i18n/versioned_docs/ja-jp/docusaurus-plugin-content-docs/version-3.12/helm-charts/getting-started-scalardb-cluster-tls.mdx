...
1. ライセンスキーと証明書を環境変数に設定します。ライセンスキーをお持ちでない場合は、[お問い合わせ](https://www.scalar-labs.com/contact)してください。`<CERT_PEM_FOR_YOUR_LICENSE_KEY>` の値の詳細については、[ライセンスキーの設定方法](../scalar-licensing/index.mdx)を参照してください。
